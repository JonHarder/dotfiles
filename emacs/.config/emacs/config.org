...
#+end_src
* Scrolling
Use the delightful ultra-scroll
...
#+end_src
* Editing
Basic editor settings, using built-ins

#+begin_src emacs-lisp :tangle "my-modules/my-editor.el" :mkdirp yes
  (delete-selection-mode 1)
  (provide 'my-editor)
#+end_src
* Toggling things (key bindings)
There are various settings I want quick access to toggling.  Either
modes to turn on and off, or appearances like themes.  Here we bind
them all under the "C-c t" prefix.

#+begin_src emacs-lisp :tangle "my-modules/my-toggles.el" :mkdirp yes
...
#+begin_src emacs-lisp :tangle "my-modules/my-modeline.el" :mkdirp yes
  ...
  #+end_src
** Miscellaneous mode line components
*** Which function mode
This minor mode adds a segment to the mode line to indicate which
enclosing ~defun~ the point is in.  It is fairly flexible and includes
indicating the currenti heading in outline compatible modes, including
org mode.
...
#+begin_src emacs-lisp :tangle "my-modules/my-time.el" :mkdirp yes
...
#+end_src
* Folding
#+begin_src emacs-lisp :tangle "my-modules/my-fold.el" :mkdirp yes
...
#+end_src
* Completion
** General (built in) completion help
*** Recentf mode
recentf mode keeps a history of files recently visited.  Pair this
with consult-buffer's file section and we can switch to recently
opened files via [[help:consult-buffer]]
...
*** Hippie expand
It's good. Better than dynamic abbrev.  See mastering emacs [[https://www.masteringemacs.org/article/text-expansion-hippie-expand][article]]
for more.
...
Cape is a small package meant to extend corfu with additional
...
#+end_src
* Nginx mode
#+begin_src emacs-lisp :tangle "my-modules/my-nginx.el" :mkdirp yes
  (straight-use-package 'nginx-mode)
  (provide 'my-nginx)
#+end_src
* Server mode
I run emacs in a server mode so that clients can connect to the
running emacs instance for short editing commands. An example of this
is ~git commit~ or ~jj describe~. I just want to open a buffer in emacs,
do my editing there, then exit.

To do this, I want to ensure emacs is running in server mode. That can
be accomplished via =server-start=

#+begin_src emacs-lisp :tangle "my-modules/my-server.el" :mkdirp yes
  (require 'server)
  
  (server-start)
#+end_src

I also want to have a nicer binding to save and close server buffers
to return control back to whatever program created the buffer.
...
* Notes
** Org-node
#+begin_src emacs-lisp :tangle "my-modules/my-notes.el" :mkdirp yes
  (use-package org-mem
    :straight t
    :after (org)
    :config
...
* Repeat mode
#+begin_src emacs-lisp :tangle "my-modules/my-repeat.el" :mkdirp yes
...
* Hyperbole
#+begin_src emacs-lisp :tangle "my-modules/my-hyperbole.el" :mkdirp yes
...
#+end_src
** Node
I use fnm to handle different node environments and manage global
package installations.  To teach emacs about this, we find and add the
fnm bin path to 'exec-path'
...
Language server client implementation. Comes baked into emacs starting
at version 2 There seems to be a bug with the latest version of
`menu-bar' (maybe?  or perhaps in `project' Eglot attempts to insert
itself info the menu bar, but the project entry in the menu bar is not
defined. Setting this variable to ~nil~ resolves the immediate issue
causing ~eglot~ to crash. It is unknown what downstream ramifications
...
** Rest mode
   ...
   #+end_src
** Starlark mode
Note, this package is archived and unmaintained.

#+begin_src emacs-lisp :tangle "my-modules/my-starlark-mode.el" :mkdirp yes
  (straight-use-package 'bazel)
  (add-to-list 'auto-mode-alist '("Tiltfile" . bazel-starlark-mode))
  (provide 'my-starlark-mode)
#+end_src
** Docker support
*** Dockerfile mode
...
#+begin_src emacs-lisp :tangle "my-modules/my-k8s.el"
...
** Custom weighted progress tracking
sometimes I want to track progress on a project with weighted values
for each sub-task.  This custom code populates a =PROGRESS= property on
the parent heading with the sum of weights of each child heading.
Child headings without a =WEIGHT= property are assumed to have a weight
of 1.
...
* Navigation
** Windows
I want easier access to `other-window' since I use it all the time
...
**** Hello sections
The following configures the "hello" (start) page of notmuch. This
page is the default view when running the command ~notmuch~ This
configures the sections on the hello page of notmuch. I don't like the
header because it doesn't provide any useful information for I also
remove the "recent searches" because these can be easily retrieved in
buffer history when executing a search with =
...
**** Sorting
   ...
   #+end_src
*** Bindings
Ensure notmuch buffers are in motion state
...
** Forge
See the amazing [[https://github.com/blahgeek/emacs-pr-review][pr-review]] package.
