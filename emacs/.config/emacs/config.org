...
disabling this for now, the jump of the cursor when attempting to open
a new line below the cursor when on a header only to have it fly to
the bottom of the screen and start a new heading is simply too much of
a jolt to handle.

Perhaps if there were another keybinding to map this to rather than =o=
...
evil-collection space as many things that seem reasonable in a vacuum,
but completely break practically all of my keybinds which rely on
space as a leader key to trigger anything and everything.
...
With that out of the way, we can set up our bindings as usual.
...
*** meta mappings
    ...
    These are meta map leader prefixed keybindings to add additional mode-aware
