...
* Epa mode
  Epa mode handles (en|de)cryption of gpg files (among others??). When saving a file
  with the extension ~.gpg~, emacs will prompt you for which key to use for encrypting
  it with.

** Bindings
   #+begin_src emacs-lisp :tangle "my-modules/my-epa.el" :mkdirp yes
