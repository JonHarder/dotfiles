...
#+end_src
** hl line mode
hl-line-mode darkens the line the point is on to help distinguish it
...
And let's use the ~lin~ package to make buffers where line selection is
the primary mode of interaction more visible.
...
#+end_src
*** Which-key mode
Which key shows available commands and their bindings when prefix
...
#+end_src
** Completion preview
this is a built in set of features with emacs version 31
...
* Diagrams
#+begin_src emacs-lisp :tangle "my-modules/my-diagrams.el" :mkdirp yes
  (use-package d2-mode
    :straight t
    :init
...
#+end_src
** Org timegrid
incredible svg backed visual calendar for org events/tasks
...
Agentic coding through the =opencode= CLI, driven from Emacs with
=emacs-opencode= and deep Emacs access via =emacs-mcp-server=.

The opencode configuration lives in =~/.config/opencode/= which is managed by
stow from =dotfiles/opencode/= where the global config (agents, permissions,
models) and the Emacs-specific config (=opencode.emacs.jsonc=, used only for
Emacs-launched servers via =OPENCODE_CONFIG=) are defined.

** request (emacs-opencode dependency)
...
** emacs-opencode client

=emacs-opencode= is a native Emacs client for the =opencode= server. Servers
launched from Emacs get the Emacs-specific opencode config (which registers
the Emacs MCP server and instructs the agent that it runs inside Emacs).
...
A pure-Elisp MCP server that exposes this Emacs instance (buffers, elisp
evaluation, diagnostics, org tools) to the opencode agent as tools prefixed
with =emacs_=. The server listens on a unix socket and is bridged to the
agent's stdio by =mcp-wrapper.py=. It starts on the startup hook so the
daemon serves the socket from boot.
...
=eval-elisp= is effectively remote code execution for the agent. The server's
security layer (dangerous-function blocklist, credential-file protection,
permission prompts) applies, but treat it as trusted. Tighten with
=mcp-server-emacs-tools-enabled= if desired.

** Keybindings

=C-c a= is the AI prefix: new sessions, one-shots, resume, status.
